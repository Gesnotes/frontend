...
export default defineConfig(({ command }) => ({
  ...
  plugins: [
    ...
    VitePWA({
      ...
      injectRegister: null,
      /**
       * `prompt` en production : on ne recharge jamais l'application sous les
       * pieds d'un parent en train de lire un bulletin sans lui demander.
       *
       * `autoUpdate` en dev (`vite serve`) : sans ça, le service worker généré
       * à chaque redémarrage du serveur reste inactif tant que personne ne
       * clique sur « Mettre à jour », et sert le bundle de la session
       * précédente — écran de chargement qui traîne, page blanche, nav qui
       * n'affiche pas les derniers écrans ajoutés. Aucun utilisateur réel
       * n'est concerné par un rechargement silencieux pendant le développement.
       */
      registerType: command === 'serve' ? 'autoUpdate' : 'prompt',
